...
- **Phase 10 — Scalable foundation (in progress):** versioned SQLite migrations, persisted session metadata, validated cursor-based event queries, a bounded virtualized Timeline, paginated inspector queries, configurable retention, malformed-record recovery, negotiated transport health, categorized drop accounting, and native WebSocket backpressure protection are complete. Remaining work covers Electron end-to-end coverage and larger sustained-load profiling.
